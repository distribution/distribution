...
| `spooldir`  | no | A local directory for pending multipart upload data, used instead of memory. By default, data is buffered in memory. |
...
`spooldir`: (optional) A local directory where pending multipart upload data is written instead of being held in memory. When unset, each in-flight upload buffers up to `chunksize` bytes in memory, so memory usage grows with concurrent pushes. The directory must exist, be writable by the registry, and have `chunksize` bytes free per concurrent upload; a tmpfs or Kubernetes `emptyDir` works well.
