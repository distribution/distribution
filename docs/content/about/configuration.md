...
| Parameter | Required | Description                                           |
|-----------|----------|-------------------------------------------------------|
| `ttl`     | yes      | Remove proxy cache configured in "storage" until storage is below this size. The suffix is one of `b`, `kb`, `mb`, `gb`, `tb`, `tb`, `pb`, `eb`. If you specify a value but omit the suffix, the value is interpreted as a number of bytes. The value must be positive. |
