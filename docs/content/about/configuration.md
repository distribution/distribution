...
The `tls` section allows you to configure client certificates for the pull-through cache to authenticate with the upstream registry when client certificate authentication is required.

| Parameter | Required | Description |
|-----------|----------|-------------|
| `certificate` | together* | Path to the client certificate file. |
| `key` | together* | Path to the client private key file. |
...
* `certificate` and `key` are optional when client certificate authentication is not needed, but if either is specified, both must be provided.
Example:
