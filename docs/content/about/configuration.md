...
| `certificatereloadinterval`  | no   | If set, the certificate and key files are reloaded from disk at this interval (e.g. `24h`, `1h30m`). Supports Go duration format. If not set, the certificate is loaded once at startup and never reloaded. Not compatible with `letsencrypt`.                                                                                                                                                     |
