...
//
// See http://ks3.ksyun.com/doc/api/multipart_upload.html for an overview of multipart uploads.
type Multi struct {
	Bucket   *Bucket
	Key      string
	UploadId string
...
//
// See http://ks3.ksyun.com/doc/api/bucket/upload.html for details.
...
//
// See http://ks3.ksyun.com/doc/api/object/upload_init.html for details.
...
func (m *Multi) PutPartCopy(n int, options CopyOptions, source string) (*CopyObjectResult, Part, error) {
	...
	sourceBucket := m.Bucket.KS3.Bucket(strings.TrimRight(strings.SplitAfterN(source, "/", 2)[0], "/"))
	...
	for attempt := attempts.Start(); attempt.Next(); {
		...
		err = m.Bucket.KS3.query(req, resp)
		if shouldRetry(err) && attempt.HasNext() {
			continue
		}
		if err != nil {
...
//
// http://ks3.ksyun.com/doc/api/object/upload_part.html for details.
...
func (m *Multi) putPart(n int, r io.ReadSeeker, partSize int64, md5b64 string) (Part, error) {
	...
	for attempt := attempts.Start(); attempt.Next(); {
		...
		err = m.Bucket.KS3.prepare(req)
		if err != nil {
			return Part{}, err
		}
		resp, err := m.Bucket.KS3.run(req, nil)
		if shouldRetry(err) && attempt.HasNext() {
			continue
		}
		if err != nil {
...
// returned.
//
// See http://ks3.ksyun.com/doc/api/object/list.html for details.
...
//
// See http://ks3.ksyun.com/doc/api/object/upload_complete.html for details.
func (m *Multi) Complete(parts []Part) error {
	...
	for attempt := attempts.Start(); attempt.Next(); {
		...
		// A 200 error code does not guarantee that there were no errors,
		// so first figure out what kind of XML "object" we are dealing with.

		if resp.XMLName.Local == "Error" {
			// KS3.query does the unmarshalling for us, so we can't unmarshal
			...
			if err := xml.Unmarshal([]byte(fullErrorXml), ks3err); err != nil {
				return err
			}
		...
		if resp.XMLName.Local == "CompleteMultipartUploadResult" {
			// One could probably add a CompleteFull method returning the
...
//
// See http://ks3.ksyun.com/doc/api/object/upload_abort.html for details.
func (m *Multi) Abort() error {
	...
	for attempt := attempts.Start(); attempt.Next(); {
		...
		err := m.Bucket.KS3.query(req, nil)
		if shouldRetry(err) && attempt.HasNext() {
			continue
		}
		return err
