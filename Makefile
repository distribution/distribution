...
GO_EXTLDFLAGS=-extldflags "-Wl,-z,now"
endif
GO_LDFLAGS=-ldflags '$(GO_EXTLDFLAGS) -s -w -X $(PKG)/version.version=$(VERSION) -X $(PKG)/version.revision=$(REVISION) -X $(PKG)/version.mainpkg=$(PKG) $(EXTRA_LDFLAGS)'
