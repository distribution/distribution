...
func TestSchedule(t *testing.T) {
	...
	var mu sync.Mutex
	s := New(context.Background(), inmemory.New(), "/ttl", time.Hour)
...
func TestRestoreOld(t *testing.T) {
	...
	s := New(context.Background(), fs, "/ttl", time.Hour)
	s.OnBlobExpire(deleteFunc)
	err = s.Start()
	if err != nil {
		t.Fatalf("Error starting ttlExpirationScheduler: %s", err)
...
func TestStopRestore(t *testing.T) {
	...
	fs := inmemory.New()
	pathToStateFile := "/ttl"
	s := New(context.Background(), fs, pathToStateFile, time.Hour)
	s.onBlobExpire = deleteFunc
	...
	s2 := New(context.Background(), fs, pathToStateFile, time.Hour)
