...
// BuildRecipeURL constructs the url for the recipe identified by name and dgst.
func (ub *URLBuilder) BuildRecipeURL(ref reference.Canonical) (string, error) {
	route := ub.cloneRoute(RouteNameRecipe)
...
// BuildRecipesURL constructs the url for the recipe identified by name and dgst.
func (ub *URLBuilder) BuildRecipesURL() (string, error) {
	route := ub.cloneRoute(RouteNameRecipes)

	layerURL, err := route.URL()
	if err != nil {
		return "", err
	}
