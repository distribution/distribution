...
func TestCatalogAPI(t *testing.T) {
	...
	for _, image := range allCatalog {
		...
		testManifestAPISchema2(t, env, imageName, "sometag")
	}
...
func TestTagsAPI(t *testing.T) {
	...
	for _, tag := range tags {
		testManifestAPISchema2(t, env, imageName, tag)
	}
...
func TestManifestAPI_DeleteTag(t *testing.T) {
	...
	tag := "latest"
	manArgs := testManifestAPISchema2(t, env, imageName, tag)
	dgst := manArgs.dgst
...
func TestRegistryAsCacheMutationAPIs(t *testing.T) {
	...
	resp := putManifest(t, "putting missing config manifest", manifestURL, schema2.MediaTypeManifest, m)
	checkResponse(t, "putting missing config manifest", resp, http.StatusBadRequest)
	_, p, counts := checkBodyHasErrorCodes(t, "putting missing config manifest", resp, v2.ErrorCodeManifestBlobUnknown)
...
func TestProxyManifestGetByTag(t *testing.T) {
	...
	manArgs := testManifestAPISchema2(t, truthEnv, imageName, tag)
	dgst := manArgs.dgst
	...
	newManArgs := testManifestAPISchema2(t, truthEnv, imageName, tag)
	newDigest := newManArgs.dgst
	if dgst == newDigest {
