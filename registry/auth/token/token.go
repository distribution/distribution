...
func (t *Token) Verify(verifyOpts VerifyOptions) (*ClaimSet, error) {
	...
	if err != nil {
		slog.Info("failed to verify token", "error", err)
	...
	if !contains(verifyOpts.TrustedIssuers, claims.Issuer) {
		slog.Info("token from untrusted issuer", "issuer", claims.Issuer)
		return nil, ErrInvalidToken
	}

	// Verify that the Audience claim is allowed.
	if !containsAny(verifyOpts.AcceptedAudiences, claims.Audience) {
		slog.Info("token intended for another audience", "audience", claims.Audience)
		return nil, ErrInvalidToken
	}
	...
	if currentTime.After(ExpWithLeeway) {
		slog.Info("token not to be used after expiration", "expiration", ExpWithLeeway, "current", currentTime)
		return nil, ErrInvalidToken
	}

	NotBeforeWithLeeway := time.Unix(claims.NotBefore, 0).Add(-Leeway)
	if currentTime.Before(NotBeforeWithLeeway) {
		slog.Info("token not to be used before its validity", "notBefore", NotBeforeWithLeeway, "current", currentTime)
		return nil, ErrInvalidToken
	}
